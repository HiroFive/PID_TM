...
PID_ATune.prototype.runtime = function () {
    ...
    if (!this.running) {
        console.log(
            'PID_Atune.runtime start sampleTime ' + this.sampleTime + ' output ' + this.output
        );
        this.peakType = 0;
        this.peakCount = 0;
    ...
    if (this.justchanged && this.peakCount > 2) {
        var avgSeparation =
            (Math.abs(this.peaks[this.peakCount - 1] - this.peaks[this.peakCount - 2]) +
                Math.abs(this.peaks[this.peakCount - 2] - this.peaks[this.peakCount - 3])) /
            2;
